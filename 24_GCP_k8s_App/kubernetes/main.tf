...
// Fetch all available GCP zones
data "google_compute_zones" "available" {}

// Fetch all available conatainer engine versions
...
locals {
  ...
  // Cluster name
  default_name = "${var.project_name_map[terraform.workspace]}-cluster"
  name         = "${local.default_name}"
  
  // 1st zone from the available zones
  primary_zone = "${data.google_compute_zones.available.names[0]}"
  
  // Set the project name we want to work with
...
//
// Create CLUSTER with empty node pool
//
resource "google_container_cluster" "primary" {
  ...
  description        = "Kubernetes cluster created using terraform"
